...
struct ModeSettings
{
  uint8_t param_select, param1, param2;
};
...
void fmtFreq(char *buf, uint16_t freqX10)
{
  ...
  else if (freqX10 < 1000)
  { // < 100 Hz: show 1 decimal
    fmtDec1(buf, freqX10);
  }
  else
  { // >= 100 Hz: integer
...
void loop()
{
  ...
  case MODE_GEN:
    runGeneratorMode(sp);
    return; // GEN handles its own display
  }
  display->display();
}
...
void setupMode(uint8_t m)
{
  stopADCSampling();
  if (genAvailable)
    dacStop(); // Always release DAC pin when switching modes
  uint8_t idx = m - 1;
  if (idx < NUM_MODES)
  {
...
void runGeneratorMode(bool showParams)
{
  ...
  if (param1 != 5 && !showParams && genDispDone)
    return; // Waveform + menu hidden → no display updates
  if (showParams)
    genDispDone = false; // User interacting → allow updates again
